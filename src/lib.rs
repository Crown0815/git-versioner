...
use git2::{Branch, BranchType, Commit, Oid, Repository, Tag};
use regex::Regex;
...
impl GitVersioner {
    ...
    fn calculate_trunk_version(&self) -> Result<Version> {
        ...
        if let Some(tag) = latest_trunk_tag {
            ...
            Ok(Version::new(0, 1, 0).with_pre_rel("rc.0")?)
        }
    }
...
mod tests {
    ...
    fn test_trunk_versioning() {
        ...
        assert_eq!(version, Version::new(0, 1, 0).with_pre_rel("rc.0").unwrap());
        
        // Add a tag and check version calculation
        test_repo.tag("v0.1.0");
        let versioner = GitVersioner::new(&test_repo.path).unwrap();
        let version = versioner.calculate_version().unwrap();
        assert_eq!(version, Version::new(0, 2, 0).with_pre_rel("rc.1").unwrap());
    }
    ...
    fn test_release_branch_versioning() {
        ...
        let versioner = GitVersioner::new(&test_repo.path).unwrap();
        let version = versioner.calculate_version().unwrap();
        assert_eq!(version, Version::new(1, 0, 1).with_pre_rel("rc.1").unwrap());
        ...
        let versioner = GitVersioner::new(&test_repo.path).unwrap();
        let version = versioner.calculate_version().unwrap();
        assert_eq!(version, Version::new(1, 0, 1).with_pre_rel("rc.2").unwrap());
        ...
        let versioner = GitVersioner::new(&test_repo.path).unwrap();
        let version = versioner.calculate_version().unwrap();
        assert_eq!(version, Version::new(1, 0, 2).with_pre_rel("rc.1").unwrap());
    }
