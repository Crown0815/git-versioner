...
#[rstest]
fn test_that_after_release_when_no_matching_tags_exist_produces_pre_release_tag_1(repo: TestRepo) {
...
fn test_that_on_release_branch_when_no_tags_exist_produces_pre_release_tag_1(repo: TestRepo) {
    ...
    repo.assert()
        .full_sem_ver("1.0.0-pre.1")
        .version_source_sha("");
}

#[rstest]
fn test_that_on_release_branch_when_release_tags_exist_produces_pre_release_tag_1(repo: TestRepo) {
    repo.branch("release/1.0.0");
    repo.commit("1.0.0+1");
    let (sha, _) = repo.tag("v1.0.0");
    repo.commit("1.0.1+1");
...
#[rstest]
fn test_that_on_release_branch_when_pre_release_tag_exist_produces_pre_release_2(repo: TestRepo) {
    repo.commit("0.1.0+1");
    let (sha, _) = repo.tag("v0.1.0-pre.1");
